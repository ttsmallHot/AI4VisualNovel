...
【分组落地规则】
1. 节点顺序应大致体现 groups 的推进顺序，但不要过于死板
2. 组间承接必须体现在跨组边上，不能断层
3. 汇合点必须有清晰逻辑来源，不要“硬汇合”
...
"""
    ...
    GAME_OUTLINE_PATCH_PROMPT = """你将收到【原大纲】和【制作人反馈】。
你的任务是只输出需要修改的字段（Partial JSON），不要重写整份大纲。

规则：
1. 仅输出发生变化的字段；未修改字段不要输出。
2. 可修改的顶层字段仅限：title, background, art_style, story_outline, characters, scenes。
3. 不得输出 story_graph。
4. 若需修改数组中的某个对象，请输出该对象的完整新值。
5. 必须输出合法 JSON 对象，不要附加任何解释。
...
"""
    ...
    STORY_GRAPH_PATCH_PROMPT = """你将收到【原 story_graph】和【制作人反馈】。
你的任务是只输出需要修改的字段（Partial JSON），不要重写整份 story_graph。

规则：
1. 仅输出发生变化的字段；未修改字段不要输出。
2. 顶层字段只允许是 nodes 和 edges。
3. 节点结构必须保持 id/summary/type，type 仅允许 normal 或 merge。
4. 必须输出合法 JSON 对象，不要附加解释。

【原 story_graph】
{previous_story_graph_json}
...
class ProducerConfig:
    ...
    SYSTEM_PROMPT = """你是一位资深，严格的 Visual Novel 游戏制作人，负责把控游戏的宏观质量和项目方向。
...
请从以下维度审核：
1. 需求契合度：是否满足用户主题、风格与核心剧情要求？是否加入了用户所要求的角色特征或设定？
2. 核心参数：groups 的规划是否足以覆盖目标节点体量？目标角色数量符合预期？
3. 结构清晰度：groups 划分是否清晰，细致
4. 剧情合理性：剧情设定是否有明显矛盾或不合理之处，是否讲述了一个完整连贯，有吸引力的故事
5. 场景与角色：场景设定是否丰富，角色设定是否有趣且多样。
...
请从以下维度审核：
1. DAG 合法性：是否无环、from/to 是否引用有效节点，是否所有节点都从 root 可达？
3. 节点数量：是否严格匹配目标总节点数
4. 分组对齐：是否体现了Step1 groups 的推进顺序与关键事件
5. 分支与汇合：汇合是否有逻辑来源，是否存在断层或硬汇合
6. 选择文本：choice_text 是否沉浸、简洁、无元标记
7. 剧情合理性：剧情设定是否有明显矛盾或不合理之处，是否讲述了一个完整连贯，有吸引力的故事？
...
**你的核心职责：**
1. 通过合理的指派丰富剧情内容，并适当推动剧情发展
2. 判断【剧情片段目标】是否已经全部达成，并找合适的时机喊停
...
<advice>该角色需要进行的大致内容,为了给接下来其他人说话的空间内容，最好只是一两句话能完成的意图，并且禁止规定具体台词，</advice>
...
class ActorConfig:
    ...
    SYSTEM_PROMPT = """你现在是 Visual Novel 游戏中的角色。
...
2. **审核要点**：
   - 图片本身逻辑是否有问题，是否存在多手/脚，低质量等图片情况, 立绘本身是否美观，符合视觉小说立绘的风格
   - 美术风格是否符合故事设定（{art_style},表情是否准确表达了 {expression} 这个情绪
   - 外貌是否符合你的设定（发色、眼睛、服装、身材等）
   - 最重要的是整体风格和气质是否符合你的人设，是否OOC（Out of Character，角色不符）
...
请直接开始审核，用你的性格和语气说话。"""
    ...
    EXPRESSION_DESCRIPTION_PROMPT = """你将收到同一角色的 neutral 立绘作为参考图。
  你的任务是描述该角色在呈现【{expression}】表情时可能的具体样貌。
  请基于参考图，提供详细的视觉描述，包含五官细节、面部神态、眼神、嘴型以及可能的肢体动作。
  描述将用于生成立绘图片。
