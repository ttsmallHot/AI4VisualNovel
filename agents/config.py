...
- **节点数量控制**：总节点数约为 {TOTAL_NODES} 个（误差 ±2 可接受）
- **分支数量灵活**：建议每个节点有 1-3 个出边，避免过于复杂
- **全员互动**：即使在特定的分支路线中，所有主要角色都应该有机会出场和互动
- **结局处理**：叶子节点（无出边）即为结局节点，包含结局剧情"""
...
{{
  ...
  "art_style": "美术风格描述（例如：赛博朋克、蒸汽朋克、日系动漫、水彩画风、像素风、写实风格等），包括色调、氛围、画面特点",
  "story_graph": {{
    "nodes": {{
      "root": {{
        "id": "root",
...
7. **建议结构**：
   - 总节点数：{total_nodes} 个（误差 ±2 可接受）
   - 汇合点：2-3 个（让不同选择最终能汇合）
   - 结局数：2-4 个（提供多种结局）
8. **路径设计与逻辑连贯性（关键！）**：
   - **汇合点必须有逻辑关联**：每条分支的线索、事件或角色决定都应该自然地收束到汇合点，而不是强行拼凑。
   - **不同分支不能毫无关联**：例如，node2 "发现美术室线索" 和 node3 "骚扰学生会" 不能在 node4 毫无理由地说"线索都指向图书馆"。这样会破坏沉浸感。
   - **汇合点应该是自然的**：可能是：(1) 两条线索交叉指向同一目标；(2) 不同角色在关键时刻集合；(3) 多个事件触发同一后果；(4) 玩家被迫在某地重逢等。
...
请从以下几个维度给出反馈：
1. **需求契合度**：方案是否完美实现了用户的所有核心要求？
2. **图完整度**：DAG结构是否合理？是否存在死节点或孤立节点？每个分支是否有意义？
3. **故事逻辑**：是否讲述了一个清晰完整的故事？各个节点剧情之间是否具有逻辑性和连贯性？是否存在冗余或重复的剧情？

请给出判断：
- 如果认为方案可以直接投入正式开发，请只回复 "PASS"。
- 如果认为需要改进，请提供具体且专业且一针见血的详细修改建议（Critique）。"""
